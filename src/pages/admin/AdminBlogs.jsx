...
const AdminBlogs = () => {
  ...
  const handleSubmitForm = async (formData) => {
    ...
    const url = isEditing 
      ? `${import.meta.env.VITE_API_URL}/blogs/${editingBlog.id}`
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || `Failed to ${isEditing ? 'update' : 'create'} blog`);
      }
    } catch (err) {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 shadow-lg shadow-green-600/20"
        >
          ...
          <table className="w-full text-left text-sm text-gray-300">
            <thead className="bg-gray-800/50 text-xs uppercase text-gray-400 border-b border-gray-700">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold hidden sm:table-cell">Date Posted</th>
            ...
            <tbody className="divide-y divide-gray-700">
              {loading ? (
                <tr>
                  <td colSpan="4" className="px-6 py-8 text-center text-gray-400">Loading blogs...</td>
                </tr>
              ) : blogs.length === 0 ? (
                <tr>
                  <td colSpan="4" className="px-6 py-8 text-center text-gray-400">No blogs found. Add one to get started!</td>
                </tr>
              ) : (
                  ...
                  <tr key={blog.id} className="hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-semibold text-white text-base">{blog.title}</div>
                          <span className="text-primary text-xs inline-block truncate max-w-[200px]">
